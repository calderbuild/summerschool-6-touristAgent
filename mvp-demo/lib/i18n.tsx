...
const DICT: Record<string, Entry> = {
  ...
  places_title: {
    ...
    fr: "Les sites parisiens, et si vous pouvez vraiment y entrer",
    zh: "巴黎景点，以及你到底进不进得去",
  },
  places_intro: {
    en: "These are the records the assistant answers from. Each says what a wheelchair user meets at the venue, whether the nearest station gets them there, and the date somebody checked. Sorted by what the data says rather than alphabetically, because the useful order is the one you can plan a day with.",
    fr: "Voici les fiches sur lesquelles l'assistant s'appuie. Chacune indique ce qu'un usager en fauteuil rencontre sur place, si la station la plus proche permet d'y arriver, et la date du dernier contrôle. Classées selon ce que disent les données plutôt que par ordre alphabétique, parce que l'ordre utile est celui avec lequel on organise une journée.",
    zh: "这些就是助手作答所依据的记录。每一条都写明轮椅使用者在现场会遇到什么、最近的车站能不能把人送到，以及最后一次核对的日期。按数据本身排序而不是按字母，因为能用来安排一天的顺序才是有用的顺序。",
  },
  places_counts: {
    en: "{total} sights: {yes} a wheelchair user can use, {depends} with a condition, {no} they cannot.",
    fr: "{total} sites : {yes} utilisables en fauteuil, {depends} sous condition, {no} inaccessibles.",
    zh: "共 {total} 处：{yes} 处轮椅可用，{depends} 处有条件，{no} 处不可用。",
  },
  places_ease_yes: { en: "Can get in", fr: "Accessible", zh: "可以进" },
  places_ease_depends: { en: "With a condition", fr: "Sous condition", zh: "有条件" },
  places_ease_no: { en: "Cannot get in", fr: "Inaccessible", zh: "进不去" },
  places_f_wheelchair: { en: "At the venue", fr: "Sur place", zh: "现场情况" },
  places_f_station: { en: "From the station", fr: "Depuis la station", zh: "从车站过去" },
  places_f_toilet: { en: "Accessible toilet", fr: "Toilettes accessibles", zh: "无障碍厕所" },
  places_f_budget: { en: "Entry", fr: "Entrée", zh: "门票" },
  places_f_hours: { en: "Hours", fr: "Horaires", zh: "开放时间" },
  places_f_transit: { en: "Nearest transit", fr: "Transports", zh: "最近的交通" },
  places_free: { en: "Free", fr: "Gratuit", zh: "免费" },
  places_checked: { en: "checked {date}", fr: "vérifié le {date}", zh: "核对于 {date}" },
  places_see_door: { en: "Look at the entrance", fr: "Voir l'entrée", zh: "看看入口" },
  places_hide_door: { en: "Hide the entrance", fr: "Masquer l'entrée", zh: "收起入口" },
  places_plan: { en: "Plan a step-free way here", fr: "Itinéraire sans marches", zh: "规划无楼梯路线" },
  places_official: { en: "Official site", fr: "Site officiel", zh: "官方网站" },
  places_ask: {
    en: "I use a wheelchair. How do I get to {place}?",
    fr: "Je me déplace en fauteuil roulant. Comment aller à {place} ?",
    zh: "我坐轮椅，怎么去{place}？",
  },
  places_sources: {
    en: "Every record names its own source and the date it was checked. Where a fact is not known it says so rather than being filled in, and a correction made by the team in the console appears here and in the assistant at the same time.",
    fr: "Chaque fiche indique sa source et sa date de contrôle. Lorsqu'un fait n'est pas connu, cela est dit plutôt que comblé, et une correction faite par l'équipe apparaît ici et dans l'assistant en même temps.",
    zh: "每条记录都写明来源和核对日期。不知道的事就说不知道，不会补上；团队在后台做的校正会同时出现在这里和助手的回答里。",
  },
  street_alt: {
    en: "Street-level 360 view of the entrance to {place}",
    fr: "Vue 360 depuis la rue de l'entrée de {place}",
    zh: "{place} 入口的街景 360 度视图",
  },
  street_dated: { en: "Google imagery, {when}", fr: "Imagerie Google, {when}", zh: "Google 影像，{when}" },
  street_caveat: {
    en: "Photographs, not a survey. Use them to find the door, not to decide it is step-free.",
    fr: "Des photos, pas un relevé. Pour trouver la porte, pas pour conclure qu'elle est sans marches.",
    zh: "这是照片，不是实地测绘。用它找到门在哪，别用它判断有没有台阶。",
  },
  street_stale: {
    en: "Old enough that the entrance may have been rebuilt since. Treat it as a landmark, not as today.",
    fr: "Assez ancienne pour que l'entrée ait pu être refaite depuis. À prendre comme repère, pas comme l'état actuel.",
    zh: "这张影像已经够旧，入口可能早已改建。把它当参照物，不要当作现在的样子。",
  },
  street_none: {
    en: "Google has no street-level imagery at this spot, so there is nothing to show. We are not substituting a view of somewhere nearby.",
    fr: "Google n'a pas d'imagerie au niveau de la rue à cet endroit : il n'y a donc rien à montrer, et nous ne mettons pas à la place une vue d'un lieu voisin.",
    zh: "Google 在这个位置没有街景影像，所以没有可展示的内容。我们不会拿附近别处的画面来顶替。",
  },
  street_failed: {
    en: "The street view did not load. The access facts above do not depend on it.",
    fr: "La vue de la rue n'a pas pu se charger. Les informations d'accessibilité ci-dessus n'en dépendent pas.",
    zh: "街景没能加载。上面的无障碍信息不依赖它。",
  },
  street_nokey: {
    en: "This build has no map key, so the street view is off. The access facts above are unaffected.",
    fr: "Cette version n'a pas de clé cartographique : la vue de la rue est désactivée. Les informations ci-dessus ne changent pas.",
    zh: "本次构建没有配置地图密钥，街景已关闭。上面的无障碍信息不受影响。",
  },
  places_link: { en: "The sights, and their doors", fr: "Les sites et leurs entrées", zh: "景点与它们的入口" },
  door_places: {
    en: "Seventeen sights with what a wheelchair user meets at the venue, the date it was checked, and a look at the entrance.",
    fr: "Dix-sept sites : ce qu'un usager en fauteuil rencontre sur place, la date du contrôle, et un aperçu de l'entrée.",
    zh: "17 处景点：轮椅使用者在现场会遇到什么、核对日期，以及入口长什么样。",
  },
  qr_link: {
